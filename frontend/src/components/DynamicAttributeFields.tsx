...
export function DynamicAttributeFields({ schema, values, onChange }: DynamicAttributeFieldsProps) {
  if (schema.length === 0) {
    return <p className="text-sm text-slate-500">Esta categoria não define campos técnicos adicionais.</p>;
  }
        ...
        <div key={field.key}>
          ...
          {field.type === "boolean" ? (
            ...
            <input
              ...
              onChange={(e) =>
                onChange(field.key, field.type === "number" ? e.target.valueAsNumber : e.target.value)
              }
              className="mt-1 w-full rounded border border-slate-300 px-3 py-2"
            />
          )}
